...
def get_country_codes(country_argument: str | Sequence[str] | None = None) -> list[str]:
    """Return validated lower-case country codes or every pycountry alpha-2 code."""

    try:
        import pycountry  # type: ignore
    except ImportError as exc:
        raise AppStoreReviewsError(
            "Не установлена зависимость pycountry. Выполните: pip install pycountry"
        ) from exc

    known = {country.alpha_2.lower() for country in pycountry.countries}
...
def export_csv(store: CheckpointStore, app_id: str, output: str | Path) -> Path:
    ...
    try:
        with temporary.open("w", encoding="utf-8-sig", newline="") as handle:
            # Russian-localized Excel expects semicolon-separated CSV fields.
