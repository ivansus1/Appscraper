...
APP_STORE_COUNTRY_CODES = tuple(
    """
    af al dz ao ai ag ar am au at az bs bh bb by be bz bj bm bt bo ba bw br vg
    bn bg bf kh cm ca cv ky td cl cn co cd cg cr ci hr cy cz dk dm do ec eg sv
    ee sz fj fi fr ga gm ge de gh gr gd gt gw gy hn hk hu is in id iq ie il it
    jm jp jo kz ke xk kw kg la lv lb lr ly lt lu mo mg mw my mv ml mt mr mu mx
    fm md mn me ms ma mz mm na nr np nl nz ni ne ng mk no om pk pw pa pg py pe
    ph pl pt qa kr ro ru rw st sa sn rs sc sl sg sk si sb za es lk kn lc vc sr
    se ch tw tj tz th to tt tn tr tm tc ug ua ae gb us uy uz vu ve vn ye zm zw
    """.split()
)
...
ProgressCallback = Callable[[int, int, str, int, RunStats], None]
...
def get_country_codes(country_argument: str | Sequence[str] | None = None) -> list[str]:
    """Return validated lower-case codes for Apple App Store storefronts."""

    known = set(APP_STORE_COUNTRY_CODES)
    if country_argument is None:
        return list(APP_STORE_COUNTRY_CODES)
    ...
    if invalid:
        raise AppStoreReviewsError(
            "Некорректные или неподдерживаемые коды App Store: "
            + ", ".join(invalid)
        )
...
class _AppleHTMLReviewParser(HTMLParser):
    """Extract only the review body from Apple's HTML-in-Atom wrapper."""

    def __init__(self) -> None:
        super().__init__(convert_charrefs=True)
        self._font_depth = 0
        self._font_parts: list[str] = []
        self.font_blocks: list[str] = []
        self.all_parts: list[str] = []

    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        name = tag.lower()
        if name == "font":
            if self._font_depth == 0:
                self._font_parts = []
            self._font_depth += 1
        elif name == "br":
            self.all_parts.append("\n")
            if self._font_depth:
                self._font_parts.append("\n")

    def handle_endtag(self, tag: str) -> None:
        if tag.lower() != "font" or not self._font_depth:
            return
        self._font_depth -= 1
        if self._font_depth == 0:
            self.font_blocks.append("".join(self._font_parts))
            self._font_parts = []

    def handle_data(self, data: str) -> None:
        self.all_parts.append(data)
        if self._font_depth:
            self._font_parts.append(data)


def extract_xml_review_text(content: str) -> str:
    """Remove Apple's HTML wrapper and return only the customer's review."""

    raw = normalize_text(content)
    if not raw or "<" not in raw:
        return raw

    parser = _AppleHTMLReviewParser()
    try:
        parser.feed(raw)
        parser.close()
    except Exception as exc:
        raise ValueError(f"не удалось разобрать HTML текста отзыва: {exc}") from exc

    # In Apple's current XML wrapper the last non-empty font block is the
    # review body. Earlier blocks contain the title and presentation metadata.
    for block in reversed(parser.font_blocks):
        candidate = normalize_text(block)
        if candidate:
            return candidate

    # Safe fallback if Apple replaces the legacy <font> structure.
...
def parse_xml_feed(payload: bytes | str, country: str = "", page: int = 1) -> PageResult:
    ...
    for index, entry in enumerate(child for child in root if _local_name(child.tag) == "entry"):
        ...
        try:
            ...
            text = extract_xml_review_text(contents[-1] if contents else "")
            if not text:
                raise ValueError("empty review text")
...
def export_csv(store: CheckpointStore, app_id: str, output: str | Path) -> Path:
    """Export exactly Дата,Оценка,Текст using Excel-friendly UTF-8 BOM."""
    ...
    try:
        with temporary.open("w", encoding="utf-8-sig", newline="") as handle:
            # Semicolon makes the three fields open as separate columns in
            # Excel installations that use the Russian locale.
            writer = csv.writer(handle, delimiter=";")
            writer.writerow(["Дата", "Оценка", "Текст"])
            for row in store.iter_reviews(app_id):
                try:
                    ...
                    LOGGER.warning(
                        "Пропущена поврежденная запись при экспорте: %s", exc
                    )
                    continue

                # Clean both newly parsed values and legacy checkpoint rows.
                text = extract_xml_review_text(str(row["text"]))
                writer.writerow([export_date, "★" * rating, text])
...
def collect_reviews(
    ...
    *,
    ...
    """Collect sequentially and optionally report structured live progress.

    Callback exceptions are deliberately not swallowed. This lets a host add
    cooperative cancellation later without coupling the collector to a UI.
    """
    ...
    def report_progress(country_index: int, country: str, page: int) -> None:
...
def run(
    ...
    *,
    ...
    """Import-friendly, non-interactive high-level entry point."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.reset:
            _confirm_reset(Path(args.checkpoint).expanduser().resolve())
        run(
            app_url,
            output=args.output,
